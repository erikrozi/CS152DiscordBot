...
def new_report_filed(completed_report, user_being_reported, user_making_report, reports_by_user, reports_about_user,
    ...
    if not is_abuse:
        ...
        if len(user_false_reports[user_making_report]) > 30:
            ...
            return False, "We did not find this post to be abusive. Please email us if you think we made a mistake."
    ...
    if report_type == ReportType.HARASSMENT_BULLYING:
        ...
        response += " Some forms of hate speech can be legally prosecuted, including libel. Please see these " \
                    "resources to see how you could potentially hold your abusers accountable under the law."
...
def spam_report(reports_about_user_list):
    ...
    if spam_count > 30:
        ...
        return True, "Your post has marked as spam and has been removed. Please email us if you think we made a mistake."


def sexual_report(report, reports_about_user_list):
    manager_review_queue.append(report) # To check if CSAM.
    return True, check_if_have_3_strikes(reports_about_user_list)
...
def check_if_have_3_strikes(reports):
    ...
    if offensive_count > 3:
        ...
        return "Your post has marked as offensive and has been removed. Please email us if you think we " \
                     "made a mistake."
...
def threatening_dangerous_report(report, bad_things_list, user_being_reported, reports_about_user):
    ...
    return True, check_if_have_3_strikes(reports_about_user[user_being_reported]) + " Please see the below mental health " \
                                                                              "resources."
...
def general_harassment_report(user_being_reported, user_making_report, reports_by_user, reports_about_user):
    ...
    if num_reports_by_user_in_last_24_hours == 0:
        return True, check_if_have_3_strikes(reports_about_user[user_being_reported])
    else:
        ...
        if number_users_being_reported == 1:
            ...
            return True, "We noticed you have reported many users for harmful content. Please click here if you " \
...
def automatic_report(bad_things, message, self, reports_about_user):
    ...
    for bad_thing in bad_things:
        if bad_thing == SEVERE_TOXICITY:
            ...
            take_post_down, response = general_harassment_report(user_being_reported, AUTOMATED, [],
                                             reports_about_user)
            response += " Some forms of hate speech can be legally prosecuted, including libel. Please see these " \
                        "resources to see how you could potentially hold your abusers accountable under the law."
            return True, response
